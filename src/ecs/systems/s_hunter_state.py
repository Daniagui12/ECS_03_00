...
def system_hunter_state(world: esper.World):
    ...
    for _, (c_v, c_a, c_ht) in components:
        if c_ht.state == HunterState.IDLE:
            _do_idle_state(c_v, c_a, c_ht)
        elif c_ht.state == HunterState.MOVE:
            _do_move_state(c_v, c_a, c_ht)

def _do_idle_state(c_v: CVelocity, c_a: CAnimation, c_ht: CHunterState):
    _set_animation(c_a, 1)
    if c_v.vel.magnitude_squared() > 0:
        c_ht.state = HunterState.MOVE

def _do_move_state(c_v: CVelocity, c_a: CAnimation, c_ht: CHunterState):
    _set_animation(c_a, 0)
    if c_v.vel.magnitude_squared() <= 0:
        c_ht.state = HunterState.IDLE
